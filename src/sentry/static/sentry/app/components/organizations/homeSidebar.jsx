...
let RouterOrBrowserLink = ({isRouter, path, ...props}) =>
  (isRouter ? <ListLink to={path} {...props} /> : <li><a href={path} {...props} /></li>);
    ...
    <div>
      ...
      <ul className="nav nav-stacked">
        ...
        {access.has('org:admin') &&
          ...
          <ListLink to={`${pathPrefix}/api-keys/`}>
            {t('API Keys')}
          </ListLink>}

        {access.has('org:write') &&
          <ListLink to={`${pathPrefix}/audit-log/`}>
            {t('Audit Log')}
          </ListLink>}
        {access.has('org:write') &&
          <ListLink to={`${pathPrefix}/rate-limits/`}>
            {t('Rate Limits')}
          </ListLink>}
        {features.has('integrations-v3') &&
          access.has('org:integrations') &&
          <ListLink to={`${pathPrefix}/integrations/`}>
            {t('Integrations')}
          </ListLink>}
        {access.has('org:write') &&
          <ListLink to={`${pathPrefix}/repos/`}>
            {t('Repositories')}
          </ListLink>}
        {access.has('org:write') &&
          <ListLink to={`${pathPrefix}/settings/`}>
            {t('Settings')}
          </ListLink>}
      </ul>
    </div>
